...
class DDPG(AbstractSolver):
    ...
    def compute_target_values(self, next_states, rewards, dones):
        ...
        gamma = self.options.gamma #Gamma
        actions_next = self.target_actor_critic.pi(next_states) # Returns the greedy action at states
        q_values_next = self.target_actor_critic.q(next_states, actions_next) # Returns the Q-values
        target_q = (1 - dones) * gamma * q_values_next + rewards
        return target_q
        ################################
    ...
    def train_episode(self):
        ...
        for _ in range(self.options.steps):
            ################################
            #   YOUR IMPLEMENTATION HERE   #
            action = self.select_action(state) # Sample an action from the policy
            next_state, reward, done, _ = self.step(action) # Performs an action in the env
    ...
    def pi_loss(self, states):
        ...
        ################################
        #   YOUR IMPLEMENTATION HERE   #
        actions = self.actor_critic.pi(states) # Returns the greedy action at states
        return -self.actor_critic.q(states, actions) # Returns the Q-values for (states, actions)
        ################################
